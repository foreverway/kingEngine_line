...
activitiModeler
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(type, modelId) {

				var modelUrl;

				// 流程解决方案
				if (type === "solution") {
					$rootScope.editType = "solution";
					modelUrl = KISBPM.URL.getSolution(modelId);
				} else {
					$rootScope.editType = "template";
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        ...
                        $rootScope.editor.setEditType($rootScope.editType);
                    }).
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
                  ...
                  var type = EDITOR.UTIL.getParameterByName("type");
                  fetchModel(type, modelId);
	                ...
	                jQuery(window).resize(function () {
						...
						var toolbarHeight = jQuery(".editor-toolbar").height() || 0;
						var canvas = jQuery('#canvasSection');
						// var mainHeader = jQuery('#main-header');

						// 流程方案配置时无工具栏影响scoll-x
						if ($rootScope.editType === 'solution') {
							var totalAvailable = jQuery(window).height();
							canvas.height(totalAvailable);
							jQuery('#propertyBottom').height(totalAvailable - 26);
							// return;
						}
	                    ...
	                    var totalAvailable = jQuery(window).height() - offset.top;
                      canvas.height(totalAvailable - toolbarHeight);
						jQuery('#paletteSection').height(totalAvailable - toolbarHeight);
						jQuery('#propertyBottom').height(totalAvailable- toolbarHeight - 26);
						// jQuery('#propertySection').height(totalAvailable - toolbarHeight);
	                    ...
	                    if(actualCanvas) {
	                        ...
	                        actualWidth = actualCanvas.parentNode.clientWidth || actualCanvas.clientWidth;
	                    }
